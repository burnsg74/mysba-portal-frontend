  ...
  try {
    ...
    sourceSansProVariants.map(async variant => {
      await fs.copy(
        path.resolve(
          __dirname,
          'node_modules/@uswds/uswds/dist/fonts/source-sans-pro/sourcesanspro-' + variant + '-webfont.woff2'
        ),
    ...
    await fs.copy(path.resolve(__dirname, 'mock-data'), path.resolve(__dirname, 'public/mock-data'));
