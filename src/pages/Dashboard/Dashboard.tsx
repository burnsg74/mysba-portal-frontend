  ...
  return (<>
      ...
      <div className={`banner ${styles["banner"]}`}>
        <div className={`${styles["welcome-message"]}`}>
          {user.profile?.crm && (<span className="username">
        ...
        <div className={`${styles["dashboard-content"]}`}>
          ...
          {user.certifications && user.certifications.map((certification, index) => {
            ...
            return (<React.Fragment key={index}>
                {certification.days_until_expiry <= 0 ? (<div className={`${styles["alert__container"]}`}>
                    <Alert
                      key={index}
                      type={"error"}
                      message={t("Your" + " " + certification.certification_type + " " + "certification has expired")}
                    />
                  </div>) : certification.days_until_expiry <= 90 ? (<div className={`${styles["alert__container"]}`}>
                    <Alert
                      key={index}
                      type="warning"
          ...
          <div className={`${styles.businessesContainer}`}>
            {user.businesses && [...user.businesses].sort((a, b) => a.name.localeCompare(b.name))
              .map((business, index) => (<React.Fragment key={index}>
                <div className={`${styles.businessContainer}`}>
                  ...
                  {user.certifications && user.certifications
                    ...
                    .map((certification, index) => (<div key={index} className={`grid-row ${styles.certificationsRow}`}>
                      <div className="grid-col">
