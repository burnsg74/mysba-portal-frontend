...
const LoanDetail = () => {
  ...
  const loan: ILoan | undefined = user.loans?.find(
    (loan: ILoan) => loan.sba_number === id
  );
  if (!loan) {
    navigate("/error");
    return null;
    ...
    <>
      <div data-testid="page-loan-details" className={`${styles.container}`}>
        ...
        <div className={`${styles.categoryGroup}`}>
          ...
          {loan.outstanding_balance !== undefined && <Field label="Outstanding Balance" value={loan.outstanding_balance} />}
          {loan.amount_to_be_current !== undefined && <Field label="Amount to be Current" value={loan.amount_to_be_current} />}
          {loan.payment_due_date && <Field label="Payment Due Date" value={loan.payment_due_date} />}
          {loan.payment_past_due !== undefined && <Field label="Payment Past Due" value={loan.payment_past_due.toString()} />}
          {loan.program_description && <Field label="Program Description" value={loan.program_description} />}
          {loan.processing_method_description && <Field label="Processing Method Description" value={loan.processing_method_description} />}
        </div>
      </div>
    </>
  );
