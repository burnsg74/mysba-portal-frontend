...
const Resources = () => {
  ...
  const handleError = (error: any, zipcode: string) => {
    const translatedMessage = error.response?.data?.message ? t(error.response.data.message, { 'Zip Code': zipcode }) : t('Error fetching local resources for given zipcode.');
  ...
  const getSocialMediaXUrlFrom = (field_district_social_media: any[]): string | null => {
    const xSocialMedia = field_district_social_media?.find(socialMedia => socialMedia.social_media_service.name === 'X');
