...
jest.mock("src/components/LearningCenterCard/LearningCenterCard", () => (props: ILearningCenterCardProps) =>
  <div>{props.learningCenter.title}</div>);
...
const wrapper = ({ children }: WrapperProps) => (<Provider store={store}>
    <MemoryRouter>
      {children}
    </MemoryRouter>
  </Provider>);
