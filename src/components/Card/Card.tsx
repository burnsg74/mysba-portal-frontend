...
const Card: React.FC<ICardProps> = props => {
  ...
  return (
    <div ref={containerRef} className={`usa-card__container ${styles.container}`}>
      <div
        className={`usa-card__header grid-row ${styles.headerRow} ${isSmallScreen ? styles.smallScreen : ''}`}
      ...
      >
        ...
        <div className={`grid-col-auto`}>
          {props.hideDetails ? null : (
            <Link to={props.detailsPage} data-testid="details-button" className={`usa-button  ${styles.detailsButton}`}>
              {t('Details')}
            </Link>
          )}
        </div>
      </div>
      <div className={` ${styles.body}  ${isSmallScreen ? styles.smallScreen : ''}`} data-testid="card-body">
        {props.body}
      </div>
    </div>
  );
