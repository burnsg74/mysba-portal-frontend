  ...
  return (<Card
      ...
      body={<div className={`grid-row ${styles.bodyRow}`} data-testid="loan-card-body">
        ...
        <div className="grid-col-auto">
          {loan.program_description === '504' ? (<div
              className={styles.bodyRowRightGroup}>{t('Amount to be Current')}: {loan.amount_to_be_current}</div>) : (
            <div className={styles.bodyRowRightGroup}>
              {t('Next Payment')}: {loan.amount_to_be_current}
              {loan.payment_due_date != null && ` ${t('Next Payment')} ${loan.payment_due_date}`}
            </div>)}
        </div>
      </div>}
    />);
