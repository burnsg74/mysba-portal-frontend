...
export const LoanCard: React.FC<ILoanCardProps> = ({ loan }) => {
  ...
  return (<Card
    ...
    body={<div className={`grid-row ${styles.bodyRow}`} data-testid="loan-card-body">
      <div className={`grid-col ${styles.bodyLoanNumber}`}>SBA Loan Number: {loan.sba_number}</div>
      <div className="grid-col-auto">
        {loan.program_description === '504' ? (
          ...
          <div className={styles.bodyRowRightGroup}>
            Next Payment: {loan.amount_to_be_current}
            {loan.payment_due_date != null && ` due on ${loan.payment_due_date}`}
          </div>)}
      </div>
    </div>}
  />);
